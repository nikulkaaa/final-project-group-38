...
def get_metric(name: str) -> "Metric":
    ...
    if name in metric_classes:
        ...
        raise ValueError(f"Metric '{name}' not found. Available metrics: {', '.join(METRICS)}")
...
class AveragePrecision(Metric):
    ...
    def __call__(self, predictions: np.ndarray, ground_truth: np.ndarray) -> float:
        ...
        # Create an array of cumulative sums of the true labels, useful for calculating precision
        cumulative_true = np.cumsum(ground_truth_sorted)

        # Calculate precision at each threshold
        precision_at_t = cumulative_true / (np.arange(len(ground_truth_sorted)) + 1)

        # Calculate recall at each threshold
        recall_at_t = cumulative_true / np.sum(ground_truth_sorted)

        # Calculate the changes in recall
        recall_change = np.diff(recall_at_t, prepend=0)  # prepend 0 to have the same length as precision

        # Calculate average precision as the sum of products of precision and recall change
...
class LogLoss(Metric):
    ...
    def __call__(self, predictions: np.ndarray, ground_truth: np.ndarray) -> float:
        """Calculates and returns the logarithmic loss to captrue confidence in predictions."""
...
class MeanSquaredError(Metric):
    """"Class to show the Mean Squared Error between predicted and actual values."""

    def __call__(self, predictions: np.ndarray, ground_truth: np.ndarray) -> float:
        """Calculates and returns the Mean Squared Error between predicted and actual values."""
        return np.sum((predictions - ground_truth)**2) / len(predictions)

class RSquared(Metric):
    """"Class to show how well predictions approximate actual values based on the R-squared statistic."""
    def __call__(self, predictions: np.ndarray, ground_truth: np.ndarray) -> float:
        """Calculates and returns the Mean Squared Error between predicted and actual values."""
...
class MeanAbsoluteError(Metric):
    """Class to calculate the Mean Absolute Error (MAE) between predicted and actual values."""
    def __call__(self, predictions: np.ndarray, ground_truth: np.ndarray) -> float:
        """Calculates and returns the Mean Absolute Error between predictions and ground truth values."""

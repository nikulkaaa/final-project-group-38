...
def load_pipeline():
    ...
    if pipelines:
        ...
        if st.button('Load Pipeline'):
            ...
            if artifact:
                try:
                    st.session_state.actual_pipeline = pickle.loads(artifact.data)
...
def determine_model_type(model_class_name: str) -> str:
    ...
    if model_class_name in ['MLPClassifierModel', 'KNearestNeighbors', 'DecisionTreeClassifierModel']:
        return "classification"
    elif model_class_name in ['MultipleLinearRegression', 'RadiusNeighborsModel', 'LassoModel']:
        return "regression"
    return "Unknown"

def generate_experiment_report(predictions, df, model_type, target_feature_name):
    """Generates experiment reports (Confusion Matrix or Actual vs Predicted)."""
...
def generate_classification_report(predictions, df, target_feature_name):
    ...
    if target_feature_name in df.columns:
        ...
        y_pred = predictions if isinstance(predictions, np.ndarray) else predictions[:, 0]
...
def generate_regression_report(predictions, df, target_feature_name):
...
def what_if_analysis(df, training_input_features):
    ...
    row_index = st.number_input("Select Row to Modify", min_value=0, max_value=len(df) - 1, value=0)
    ...
    for feature in training_input_features:
        if feature in modified_row:
            ...
            modified_row[feature] = st.slider(f"Adjust {feature} (row {row_index})", min_value=min_value, max_value=max_value, value=default_value)

    st.write("Modified Row Preview:")
    st.dataframe(modified_row.to_frame().T)

    modified_data = pd.DataFrame(modified_row).T
    modified_dataset = Dataset.from_dataframe(modified_data, name="modified_data", asset_path="modified_data.csv")
    st.session_state.actual_pipeline.dataset = modified_dataset

    if st.button('Re-run Predictions with Modified Data'):
        try:
            modified_predictions = st.session_state.actual_pipeline.predict(modified_dataset)
            ...
            generate_experiment_report(modified_predictions, modified_data, model_type, target_feature_name)
        except Exception as e:
...
def upload_and_predict():
    ...
    if uploaded_file is not None:
        ...
        if set(training_input_features) != set(uploaded_columns):
            error = "Please upload a dataset with the same column headers as the one used for training."
            st.error(error)
            ...
            if st.button('Run Predictions'):
                try:
                    ...
                    error_message = f"{e}. Please ensure the dataset matches the trained model's type."
